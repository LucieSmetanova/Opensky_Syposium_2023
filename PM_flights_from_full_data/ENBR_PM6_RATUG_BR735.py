...
RATUG_lat, RATUG_lon = (60.067539,5.745203)
RATUG_circle_center = Point(RATUG_lon, RATUG_lat)

BR735_lat, BR735_lon = (60.053169,5.90895)
BR735_circle_center = Point(BR735_lon, BR735_lat)
...
for flight_id, flight_df in states_df.groupby(level='flightId'):
    ...
    for seq, row in flight_df.groupby(level='sequence'):
        ...
        if (check_circle_contains_point(BR735_circle_center, radius, Point(lon, lat))):
            step = 1
        if (check_circle_contains_point(RATUG_circle_center, radius, Point(lon, lat))):
            if step == 1:
                drop = False
                #print(seq)
                break
...
filename = "PM_dataset_arrival_50NM_RATUG_BR735.csv"
