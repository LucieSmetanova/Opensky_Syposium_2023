...
IBLIR_lat, IBLIR_lon = (59.992131,4.869267)
IBLIR_circle_center = Point(IBLIR_lon, IBLIR_lat)

BR725_lat, BR725_lon = (59.950844,4.725358)
BR725_circle_center = Point(BR725_lon, BR725_lat)
...
for flight_id, flight_df in states_df.groupby(level='flightId'):
    ...
    for seq, row in flight_df.groupby(level='sequence'):
        ...
        if (check_circle_contains_point(BR725_circle_center, radius, Point(lon, lat))):
            step = 1
        if (check_circle_contains_point(IBLIR_circle_center, radius, Point(lon, lat))):
            if step == 1:
                drop = False
                #print(seq)
                break
...
filename = "PM_dataset_arrival_50NM_IBLIR_BR725.csv"
