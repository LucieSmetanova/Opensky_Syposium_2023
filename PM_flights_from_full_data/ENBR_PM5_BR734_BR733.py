...
BR734_lat, BR734_lon = (60.179469,5.705108)
BR734_circle_center = Point(BR734_lon, BR734_lat)

BR733_lat, BR733_lon = (60.125269,5.744909)
...
for flight_id, flight_df in states_df.groupby(level='flightId'):
    ...
    for seq, row in flight_df.groupby(level='sequence'):
        ...
        if (check_circle_contains_point(BR734_circle_center, radius, Point(lon, lat))):
            #step = 1
        	    drop = False
        	    break
        #if (check_circle_contains_point(BR733_circle_center, radius, Point(lon, lat))):
            #if step == 1:
                #drop = False
                #print(seq)
                #break
...
filename = "PM_dataset_arrival_50NM_BR734_BR733.csv"
