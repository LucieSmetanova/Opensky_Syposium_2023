...
BR634_lat, BR634_lon = (60.404206,4.72565)
BR634_circle_center = Point(BR634_lon, BR634_lat)

BR633_lat, BR633_lon = (60.458531,4.680142)
...
for flight_id, flight_df in states_df.groupby(level='flightId'):
    ...
    for seq, row in flight_df.groupby(level='sequence'):
        ...
        if (check_circle_contains_point(BR634_circle_center, radius, Point(lon, lat))):
            #step = 1
            drop = False
            #print(seq)
            break
        #if (check_circle_contains_point(BR633_circle_center, radius, Point(lon, lat))):
            #if step == 1:
                #drop = False
                #print(seq)
                #break
...
filename = "PM_dataset_arrival_50NM_BR634_BR633.csv"
