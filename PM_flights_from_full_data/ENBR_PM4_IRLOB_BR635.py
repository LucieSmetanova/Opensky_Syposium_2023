...
IRLOB_lat, IRLOB_lon = (60.516256,4.686572)
IRLOB_circle_center = Point(IRLOB_lon, IRLOB_lat)

BR635_lat, BR635_lon = (60.530828,4.520564)
...
for flight_id, flight_df in states_df.groupby(level='flightId'):
    ...
    for seq, row in flight_df.groupby(level='sequence'):
        ...
        if (check_circle_contains_point(BR635_circle_center, radius, Point(lon, lat))):
            step = 1
        if (check_circle_contains_point(IRLOB_circle_center, radius, Point(lon, lat))):
            if step == 1:
                drop = False
                #print(seq)
                break
...
filename = "PM_dataset_arrival_50NM_IRLOB_BR635.csv"
