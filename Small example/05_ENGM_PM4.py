...
for week in [1]:
    for rwy in ['_rwy19L','_rwy19R']:
        ...
        states_df_1 = pd.read_csv(os.path.join('Output', filename), sep=' ',
        ...
        states_df = pd.concat([states_df,states_df_1],ignore_index=True)
...
GM429_circle_center = Point(GM429_lon, GM429_lat)

GM432_lat, GM432_lon = (60.761417,10.977667)
GM432_circle_center = Point(GM432_lon, GM432_lat)
...
for flight_id, flight_df in states_df.groupby(level='flightId'):
    ...
    for seq, row in flight_df.groupby(level='sequence'):
        ...
        if (check_circle_contains_point(GM429_circle_center, radius, Point(lon, lat))):
            drop = False
        elif (check_circle_contains_point(GM432_circle_center, radius, Point(lon, lat))):
            drop = False
...
filename = "PM_dataset_arrival_50NM_GM429_GM432_rwy19.csv"
states_df.to_csv(os.path.join('Output', filename), sep=' ', encoding='utf-8', float_format='%.3f', index = True, header = False)
