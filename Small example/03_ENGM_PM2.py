...
GM405_circle_center = Point(GM405_lon, GM405_lat)

GM410_lat, GM410_lon = (59.743583,10.41975)
GM410_circle_center = Point(GM410_lon, GM410_lat)
...
for flight_id, flight_df in states_df.groupby(level='flightId'):
    ...
    for seq, row in flight_df.groupby(level='sequence'):
        ...
        if (check_circle_contains_point(GM405_circle_center, radius, Point(lon, lat))):
            drop = False
        elif (check_circle_contains_point(GM410_circle_center, radius, Point(lon, lat))):
            drop = False
...
filename = "PM_dataset_arrival_50NM_GM405_GM410_rwy01.csv"
